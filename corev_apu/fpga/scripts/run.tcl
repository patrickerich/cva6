...
if {$::env(BOARD) eq "genesys2"} {
    add_files -fileset constrs_1 -norecurse constraints/genesys-2.xdc
      ...
      add_files -fileset constrs_1 -norecurse constraints/axku5.xdc
} else {
      exit 1
}

# IP cores: for AXKU5 we deliberately skip the 7-series MIG to avoid
# device mismatch issues. All other boards still use the original list.
if {$::env(BOARD) eq "axku5"} {
  read_ip { \
...
if {$::env(BOARD) eq "genesys2"} {
    ...
    set registers "../../vendor/pulp-platform/common_cells/include/common_cells/registers.svh"
      ...
      read_verilog -sv {src/axku5.svh ../../vendor/pulp-platform/common_cells/include/common_cells/registers.svh}
      set file "src/axku5.svh"
      set registers "../../vendor/pulp-platform/common_cells/include/common_cells/registers.svh"
} else {
    exit 1
}
